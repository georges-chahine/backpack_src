...
protected:
    ...
    //                    ros::Time::now(),"imu", "base_link"));
        ...
        tf::Quaternion qCam3 = tf::createQuaternionFromRPY(pi+0.436332, 0*-0.72345, pi/2);
        broadcaster.sendTransform(
                    tf::StampedTransform(
        ...
        tf::Quaternion qLaser = tf::createQuaternionFromRPY(pi, -0.3880617, 0);
        broadcaster.sendTransform(
                    tf::StampedTransform(
  ...
  */
    ...
    void GPSCallback4(const sensor_msgs::NavSatFix::ConstPtr& msg)
    {
        ...
        if ( (msg->status.status==1) && ( (msg->latitude)>0) ){
            ...
            broadcaster.sendTransform(
                        tf::StampedTransform(
                            tf::Transform(q00, tf::Vector3(x-x_init,y-y_init, -0.3)),
                            ros::Time::now(),"gps_start", "gps_current"));
            //   ROS_INFO("UTM coordinates z: [%f]", utm_pt.altitude);
        ...
        else
        {
            ...
            broadcaster.sendTransform(
                        tf::StampedTransform(
                            tf::Transform(q00, tf::Vector3(0,0, -0.3)),
                            ros::Time::now(),"gps_start", "gps_current"));
        }
