...
protected:
    ...
    //                    ros::Time::now(),"imu", "base_link"));
        ...
        tf::Quaternion qCam1 = tf::createQuaternionFromRPY(-pi/2, pi, 0*pi/2);  //was -pi/2  pi
        broadcaster.sendTransform(
                    tf::StampedTransform(
        ...
        tf::Quaternion qCam3 = tf::createQuaternionFromRPY(pi-0.72345, 0*-0.72345, pi/2);
        broadcaster.sendTransform(
                    tf::StampedTransform(
        ...
        broadcaster.sendTransform(
                    tf::StampedTransform(
                        tf::Transform(qLaser, tf::Vector3(-0.0165, 0.065, -0.063)),
