...
protected:
    ...
    //                    ros::Time::now(),"imu", "base_link"));
        ...
        tf::Quaternion qCam1 = tf::createQuaternionFromRPY(pi/2, 0*pi, pi);  //was -pi/2  pi
        broadcaster.sendTransform(
                    tf::StampedTransform(
        ...
        tf::Quaternion qCam3 = tf::createQuaternionFromRPY(0, -pi-0.72345, 0);
        broadcaster.sendTransform(
                    tf::StampedTransform(
        ...
        broadcaster.sendTransform(
                    tf::StampedTransform(
                        tf::Transform(qLaser, tf::Vector3(-0.0165, 0.05958, -0.06)),
  ...
  */
    ...
    void GPSCallback4(const sensor_msgs::NavSatFix::ConstPtr& msg)
    {
        ...
        if ( (msg->status.status==1) && ( (msg->latitude)>0) ){
            ...
            geo_pt.altitude = msg->altitude;
            geodesy::UTMPoint utm_pt(geo_pt);
            double y=utm_pt.northing;    //NED
            double x=utm_pt.easting;
            ...
            broadcaster.sendTransform(
                        tf::StampedTransform(
                            tf::Transform(q0*q1, tf::Vector3(x-x_init,y-y_init, 0.3)),
                            ros::Time::now(),"gps_start", "gps_current"));
            //   ROS_INFO("UTM coordinates z: [%f]", utm_pt.altitude);
        ...
        else
        {
            ...
            broadcaster.sendTransform(
                        tf::StampedTransform(
                            tf::Transform(q0*q1, tf::Vector3(0,0, 0.3)),
                            ros::Time::now(),"gps_start", "gps_current"));
        }
