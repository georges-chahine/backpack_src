...
protected:
    ...
    void IMUCallback(const sensor_msgs::Imu::ConstPtr& msg)
    {
       ...
       // ROS_INFO("Imu Orientation roll: [%f], pitch: [%f], yaw: [%f]", roll, pitch, yaw);
        ...
        tf::Quaternion qBaseLink = tf::createQuaternionFromRPY(0, 0, 0);
        broadcaster.sendTransform(
                    tf::StampedTransform(
                        tf::Transform(qBaseLink, tf::Vector3(0,0,0.3)),
    ...
    //                    ros::Time::now(),"imu", "base_link"));
        ...
        tf::Quaternion qCamTheolite = tf::createQuaternionFromRPY(0, 0, 0);  //was -pi/2  pi
        broadcaster.sendTransform(
                    tf::StampedTransform(
                        tf::Transform(qCamTheolite, tf::Vector3(0.56, -0.14, -0.04 )),
                        ros::Time::now(),"imu", "trimble"));
        ...
        tf::Quaternion qCam3 = tf::createQuaternionFromRPY(pi-0.436332, 0*pi+0*-0.72345, -pi/2);
        broadcaster.sendTransform(
                    tf::StampedTransform(
                        tf::Transform(qCam3, tf::Vector3(0.1185, -0.05958, -0.03504)),
                        ros::Time::now(),"imu", "camera1"));


        tf::Quaternion qLaser = tf::createQuaternionFromRPY(0, pi-0.3880617, 0);
        broadcaster.sendTransform(
                    tf::StampedTransform(
                        tf::Transform(qLaser, tf::Vector3(0.036, -0.05958, -0.06413)),
  ...
  */
    ...
    void GPSCallback4(const sensor_msgs::NavSatFix::ConstPtr& msg)
    {
        ...
        if ( (msg->status.status==1) && ( (msg->latitude)>0) ){
            ...
            broadcaster.sendTransform(
                        tf::StampedTransform(
                            tf::Transform(q, tf::Vector3(x-x_init,y-y_init, -0.3)),
